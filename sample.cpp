...
void
OsuSphereDist(float radius, int slices, int stacks)
{
	// set the globals:
	...
	for (int ilat = 0; ilat < SphNumLats; ilat++)
	{
											...
											// ilng=SphNumLngs-1, lng=+M_PI are the same meridian
		{
			...
			p->ny = y;
			p->nz = z;
			p->s = (lng + M_PI) / (sin(2.) * M_PI);
			p->t = (lat + M_PI / sin(2.)) / M_PI;
		}
	}
...
const char *WINDOWTITLE = "Lighting Example - Valor Nash";
...
GLuint	O_Sphere;				// OSU_Sphere
GLuint	O_Sphere_dist;			// OSU_Sphere Distorted
int		MainWindow;				// window id for main graphics window
float	Scale;					// scaling factor
int		ShadowsOn;				// != 0 means to turn shadows on
int		WhichColor;				// index into Colors[ ]
int		TextureOn;				// Texture
...
Display( )
{
	...
	gluLookAt( -15.f, 5.f, -10.f,     0.f, 0.f, 0.f,     0.f, 1.f, 0.f );
...
#define RAD 7.f
	...
	// light one - stationary, red, point
	glPushMatrix();
		SetPointLight(GL_LIGHT1, -5, 5, 0.0, 1., 0., 0.);
		if (Light1On)
			glEnable(GL_LIGHT1);
		else
			glDisable(GL_LIGHT1);
	...
	// light one - stationary, cyan, spot light
	glPushMatrix();
		SetSpotLight(GL_LIGHT2, 3.5, 0, -3.5, -3.5, 0., 3.5, 0., 1., 1.);
		if (Light2On)
			glEnable(GL_LIGHT2);
		else
			glDisable(GL_LIGHT2);
	...
	// Earth  -  need to make shiny and flat
		glPushMatrix();
			SetMaterial(1.0f, 1.f, 1.0f, 45.f);

			glEnable(GL_TEXTURE_2D);
			glBindTexture(GL_TEXTURE_2D, WorldTex);
			glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);

			glShadeModel(GL_FLAT);					// set surface type
	...
	// Satelite  
		glPushMatrix();
			SetMaterial(0.1f, 1.f, 0.1f, 0.f);

			glScaled(.5, .5, .5);
			glTranslatef(0, 0, 0.0); 
			glRotatef(360. * Time, 0.0, 0.0, 1.0); 
			glTranslatef(-12, -12, 0.0); 
			glShadeModel(GL_SMOOTH);				// set surface type
			glColor3f(0., 0., 0.);
			glCallList(O_Sphere);
		glPopMatrix();
...
InitLists( )
{
	...
	O_Sphere = glGenLists( 1 );
	glNewList( O_Sphere, GL_COMPILE );
		OsuSphere(2., 16, 16);
	glEndList( );

	O_Sphere_dist = glGenLists( 1 );
	glNewList(O_Sphere_dist, GL_COMPILE);
		OsuSphereDist(2., 16, 16);
	glEndList();
